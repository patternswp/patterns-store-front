 ...
 * @subpackage Patterns_Store_Front/Patterns_Store_Front_Info
 */
 ...
 */
class Patterns_Store_Front_Info {
	...
	public function add_theme_menu() {
		$this->hook_suffix[] = add_theme_page( esc_html__( 'Theme Info', 'patterns-store-front' ), esc_html__( 'Theme Info', 'patterns-store-front' ), 'edit_theme_options', PATTERNS_STORE_THEME_THEME_NAME, array( $this, 'info_screen' ) );
	}
	...
	public function enqueue_resources( $hook_suffix ) {
		...
		$unique_id = PATTERNS_STORE_THEME_THEME_NAME . '-info';
		...
		$deps_file = PATTERNS_STORE_THEME_PATH . 'build/admin/info/info.asset.php';
		...
		wp_enqueue_script( $unique_id, PATTERNS_STORE_THEME_URL . 'build/admin/info/info.js', $dependency, $version, true );

		wp_enqueue_style( $unique_id, PATTERNS_STORE_THEME_URL . 'build/admin/info/info.css', array(), $version );
		wp_style_add_data( $unique_id, 'rtl', 'replace' );

		/* Localize */
		$localize = apply_filters(
			'patterns_store_front_info_localize',
			array(
				'version'  => $version,
...
/**
 * Return instance of Patterns_Store_Front_Info class
 *
 * @since 1.0.0
 *
 * @return Patterns_Store_Front_Info
 */
function patterns_store_front_info() { //phpcs:ignore
	return Patterns_Store_Front_Info::instance();
}
patterns_store_front_info()->run();
