...
class PatternsStoreFrontSettingsApi {
	...
	bindEvents() {
		document.addEventListener( 'click', ( event ) => {
			...
			if ( target.closest( '.patternswp-psf-gsn-close' ) ) {
				this.modifySettings( 'hide_get_started_notice', true );
			} else if (
				target.closest( '.patternswp-psf-rn-permanent-close' )
			) {
				this.modifySettings( 'remove_review_notice_permanently', true );
			} else if (
				target.closest( '.patternswp-psf-rn-temporary-close' )
			) {
				this.modifySettings(
				...
				const installBtn = target.closest(
					'.patternswp-psf-install-plugins'
				);
				if ( installBtn ) {
					installBtn.classList.add( 'patternswp-psf-processing' );
					this.installPlugins();
				}
			}
		} );
	...
	updateUI() {
		if ( this.settings.hide_get_started_notice ) {
			const noticeElement =
				document.getElementById( 'patternswp-psf-gsn' );
			if ( noticeElement ) {
